...
def send_notification(sender, instance, **kwargs):
    ...
    if instance.reply_to is None:
        ...
        if instance.content_type.model == 'bbs':
            ...
            verb = '{0} 评论了你的帖子 《{1}》'.format(instance.user.get_nickname_or_username(), bbs.title)
        elif instance.content_type.model == 'barter':
            barter = instance.content_object
            verb = '{0} 评论了你的换品 《{1}》'.format(instance.user.get_nickname_or_username(), barter.name)
        else:
            raise Exception('unkown instance object type')
    else:
        # 回复
